...
export async function sendVerificationEmail({ user, req, kind = "user" }) {
  ...
  return sendMail({
    ...
    html: `<p>Welcome${user.firstName ? `, ${escapeHtml(user.firstName)}` : ""}!</p><p>Confirm this is your email address to activate your account. This link expires in 24 hours.</p><p><a href="${verifyUrl}">Verify email</a></p>`,
  });
}
