...
// Display-only - "Lagos" -> "Lagos State", matching how Nigerians
// actually refer to a state (used on product location lines, see
// components/MarketplaceGrid.js and StorefrontProductGrid.js). "Federal
// Capital Territory" is already a full name, not a "<name> State" one,
// so it's left as-is rather than becoming "Federal Capital Territory
// State".
export function formatStateLabel(state) {
  if (!state) return state;
  return state === "Federal Capital Territory" ? state : `${state} State`;
}
