...
export async function getUser(req) {
  ...
  if (!token) {
    console.error(`getUser: no bearer token on ${req.method} ${req.url}`);
    return null;
  }
  ...
  try {
    ...
    console.error(`getUser: jwt.verify failed on ${req.method} ${req.url}:`, err.message);
    return null;
  }

  const [user] = await db.select().from(users).where(eq(users.id, payload.userId)).limit(1);
  if (!user || user.isBanned || user.deletedAt) {
    console.error(`getUser: no matching/active user for userId ${payload.userId} on ${req.method} ${req.url}`);
    return null;
  }
