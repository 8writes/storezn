import { z } from "zod";

export function validate(schema, data) {
  const result = schema.safeParse(data);
  if (!result.success) {
    return { ok: false, error: result.error.issues[0]?.message || "Invalid input" };
  }
  return { ok: true, data: result.data };
}

// Signup email: a normal address with no "+tag" alias in the local part.
// Plus-addressing is the main lever for farming many accounts off one
// mailbox, and a real user can just give their plain address. Normalised
// dedupe (lib/emailNormalize.js) is the backstop for anything that slips
// through (e.g. an address a super-admin adds by hand).
export const signupEmail = z
  .string()
  .trim()
  .toLowerCase()
  .email("Valid email is required")
  .refine((e) => !(e.split("@")[0] || "").includes("+"), {
    message: "Please sign up with your plain email address, without a + tag.",
  });

// Bare hostname only - no protocol, no path, no port. Deliberately
// rejects storezn.com and *.storezn.com themselves (see the route) so a
// vendor can't point their "custom" domain at the platform's own root.
const DOMAIN_PATTERN = /^(?!-)[a-z0-9-]{1,63}(?<!-)(\.(?!-)[a-z0-9-]{1,63}(?<!-))+$/;

export const setCustomDomainSchema = z.object({
  customDomain: z
    .string()
    .trim()
    .toLowerCase()
    .max(253)
    .regex(DOMAIN_PATTERN, "Enter a valid domain, e.g. example.com")
    .nullable(),
});

export const loginSchema = z.object({
  email: z.string().trim().toLowerCase().email("Valid email is required"),
  password: z.string().min(1, "Password is required"),
});

export const forgotPasswordSchema = z.object({
  email: z.string().trim().toLowerCase().email("Valid email is required"),
});

export const resetPasswordSchema = z.object({
  token: z.string().trim().min(1, "Reset token is required"),
  password: z.string().min(8, "Password must be at least 8 characters"),
});

export const verifyEmailSchema = z.object({
  token: z.string().trim().min(1, "Verification token is required"),
});

export const resendVerificationSchema = z.object({
  email: z.string().trim().toLowerCase().email("Valid email is required"),
});

// Subdomains a store must never take, because the platform itself
// answers on them. "www" is the one that actually breaks: proxy.js and
// lib/resolveStore.js both treat www.<rootDomain> as the platform's own
// host, so a store whose slug is "www" is created successfully and is then
// permanently unreachable. The rest are infrastructure names a store
// shouldn't be able to squat.
const RESERVED_STORE_SLUGS = new Set([
  "www", "api", "app", "admin", "mail", "smtp", "imap", "pop", "ftp", "cdn",
  "static", "assets", "img", "images", "media", "files", "dashboard", "help",
  "support", "status", "blog", "docs", "dev", "staging", "test", "demo",
  "marketplace", "store", "stores", "storezn", "account", "accounts", "auth",
  "login", "signup", "billing", "pay", "payments", "checkout", "webhook",
  "webhooks", "cron", "ns", "ns1", "ns2", "mx", "email", "internal",
]);

// Public self-signup: stores only get created this way (no super_admin
// onboarding form) - includes a required terms-of-service acceptance,
// since the vendor themselves is the one clicking it.
export const vendorSignupSchema = z.object({
  name: z.string().trim().min(1, "Business name is required"),
  // No hyphens (unlike slugField, used for product/category slugs below) -
  // this becomes the store's actual subdomain (<slug>.storezn.com, see
  // lib/storeUrl.js), where a run of hyphens is easier to mistype/
  // misremember than in a product URL path segment.
  slug: z
    .string()
    .trim()
    .min(2, "Slug must be at least 2 characters")
    .regex(/^[a-z0-9]+$/, "Slug can only contain lowercase letters and numbers")
    .toLowerCase()
    .refine((slug) => !RESERVED_STORE_SLUGS.has(slug), "That store address is reserved - please choose another"),
  // Where the store ships from - see stores.state. Required at signup so
  // it's never missing for a new store; older stores that predate this
  // field just show no location until the vendor sets one from settings.
  state: z.string().trim().min(1, "State is required"),
  vendor: z.object({
    firstName: z.string().trim().min(1, "First name is required"),
    lastName: z.string().trim().min(1, "Last name is required"),
    email: signupEmail,
    password: z.string().min(8, "Password must be at least 8 characters"),
  }),
  acceptTerms: z.literal(true, {
    message: "You must accept the Terms of Service to continue",
  }),
  acceptMarketing: z.boolean().optional(),
});

// What a vendor themselves may edit on their own store - deliberately
// excludes isActive/commissionRatePercent, which stay super_admin-only
// (see updateStoreStatusSchema).
export const updateVendorStoreSchema = z.object({
  logoUrl: z.string().trim().url().optional().or(z.literal("")),
  faviconUrl: z.string().trim().url().optional().or(z.literal("")),
  feeChargedToCustomer: z.boolean().optional(),
  defaultShippingFee: z.coerce.number().min(0).optional(),
  defaultShippingIsTBD: z.boolean().optional(),
  returnWindowDays: z.coerce.number().int().min(0).max(365).optional(),
  address: z.string().trim().max(300).optional().or(z.literal("")),
  state: z.string().trim().optional().or(z.literal("")),
  description: z.string().trim().max(240).optional().or(z.literal("")),
  isOpen: z.boolean().optional(),
  listOnMarketplace: z.boolean().optional(),
  showShipsFrom: z.boolean().optional(),
  showDescription: z.boolean().optional(),
  // Storezn+ only (enforced server-side, see PATCH /api/v1/vendor/stores/
  // [storeId] - a free store's PATCH is rejected before this ever gets
  // written) - hex color driving the storefront's --store-accent CSS var.
  storefrontAccentColor: z
    .string()
    .trim()
    .regex(/^#[0-9a-fA-F]{6}$/, "Enter a valid hex color, e.g. #7c3aed")
    .optional()
    .or(z.literal("")),
  socialLinks: z
    .object({
      website: z.string().trim().url("Invalid URL").max(300).optional().or(z.literal("")),
      instagram: z.string().trim().url("Invalid URL").max(300).optional().or(z.literal("")),
      twitter: z.string().trim().url("Invalid URL").max(300).optional().or(z.literal("")),
      facebook: z.string().trim().url("Invalid URL").max(300).optional().or(z.literal("")),
      tiktok: z.string().trim().url("Invalid URL").max(300).optional().or(z.literal("")),
      // Bare phone number, not a URL - see stores.socialLinks in
      // lib/db/schema.js.
      whatsapp: z
        .string()
        .trim()
        .regex(/^\+?\d{7,15}$/, "Enter a valid phone number, digits only")
        .optional()
        .or(z.literal("")),
    })
    .partial()
    .optional(),
});

export const linkPayoutAccountSchema = z.object({
  bankCode: z.string().trim().min(1, "Select your bank"),
  accountNumber: z
    .string()
    .trim()
    .regex(/^\d{10}$/, "Account number must be 10 digits"),
});

export const updateProfileAndNotificationsSchema = z.object({
  firstName: z.string().trim().min(1).optional(),
  lastName: z.string().trim().min(1).optional(),
  // Optional - an empty string clears it (the route maps "" -> null).
  phone: z.string().trim().max(30).optional().or(z.literal("")),
  emailNotificationsEnabled: z.boolean().optional(),
});

export const changePasswordSchema = z.object({
  currentPassword: z.string().min(1, "Current password is required"),
  newPassword: z.string().min(8, "New password must be at least 8 characters"),
});

export const updateStoreStatusSchema = z.object({
  isActive: z.boolean().optional(),
  commissionRatePercent: z.coerce.number().min(0).max(100).nullable().optional(),
  subscriptionPriceOverride: z.coerce.number().min(0).nullable().optional(),
  // Support-driven action: clears the vendor's linked bank account so
  // they can link a new one (see the vendor payout-account POST route,
  // which otherwise refuses to overwrite an already-linked account).
  unlockPayoutAccount: z.literal(true).optional(),
});

// A Storezn+ activation for a store that paid outside the platform
// (bank transfer, cash). Records the amount so it still shows in the
// subscription-transactions ledger, and grants Plus for `months` from
// now (or from the current expiry if still active) with no auto-renew.
export const manualPlusActivationSchema = z.object({
  amount: z.coerce.number().positive("Enter the amount they paid"),
  months: z.coerce.number().int().min(1).max(36).default(1),
  // Which tier the off-platform payment buys. Enterprise is a superset of
  // Plus (adds the in-person point-of-sale suite) and is only ever
  // granted here - there's no self-serve checkout for it.
  plan: z.enum(["plus", "enterprise"]).default("plus"),
  paidAt: z.coerce.date().optional(),
  note: z.string().trim().max(300).optional(),
});

export const updatePlatformSettingsSchema = z.object({
  defaultCommissionRatePercent: z.coerce.number().min(0).max(100).optional(),
  // null means "no cap" - the client sends null explicitly to clear it
  // (not "", which z.coerce.number() would silently read as 0).
  maxCommissionAmount: z.number().min(0).nullable().optional(),
  defaultFlatFee: z.coerce.number().min(0).optional(),
  maintenanceMode: z.boolean().optional(),
  plusMonthlyPrice: z.coerce.number().min(0).optional(),
  plusIntroDiscountPercent: z.coerce.number().min(1).max(99).nullable().optional(),
  freeStorageMb: z.coerce.number().int().min(0).optional(),
  plusStorageMb: z.coerce.number().int().min(0).optional(),
  freeStaffLimit: z.coerce.number().int().min(0).optional(),
  plusStaffLimit: z.coerce.number().int().min(0).optional(),
  freeBranchLimit: z.coerce.number().int().min(1).optional(),
  plusBranchLimit: z.coerce.number().int().min(1).optional(),
});

const slugField = z
  .string()
  .trim()
  .min(1, "Slug is required")
  .regex(/^[a-z0-9-]+$/, "Slug can only contain lowercase letters, numbers, and hyphens")
  .toLowerCase();

const optionalTrimmedText = (schema) =>
  z.preprocess(
    (v) => (typeof v === "string" && v.trim() === "" ? undefined : v),
    schema.optional(),
  );

const optionalId = (message = "Select an option") =>
  optionalTrimmedText(z.string().trim().min(1, message));

const optionalEmail = (message = "Valid email is required") =>
  optionalTrimmedText(z.string().trim().toLowerCase().email(message));

export const createCategorySchema = z.object({
  name: z.string().trim().min(1, "Category name is required"),
  slug: slugField,
});

export const updateCategorySchema = createCategorySchema.partial();

// city omitted/empty means this rate applies to the whole state - see
// stores.shippingRates in lib/db/schema.js and lib/shipping.js.
export const createShippingRateSchema = z.object({
  state: z.string().trim().min(1, "State is required").max(100),
  city: z.string().trim().max(100).optional().or(z.literal("")),
  fee: z.coerce.number().min(0, "Fee can't be negative"),
});

// Structured product size chart - measurement columns (e.g. Bust, Waist)
// plus one row per size, each carrying exactly one value per column, and
// a unit the storefront's cm/in toggle converts from. See
// components/ui/SizeGuideEditor.js and SizeGuideButton.js.
export const sizeGuideSchema = z
  .object({
    unit: z.enum(["cm", "in"]),
    // Names of the sizing systems. sizeLabel heads the primary "size"
    // column (e.g. "UK"); altLabel, if set, is a second system shown in
    // parens on the storefront size buttons (e.g. "EUR") and as its own
    // column in the chart.
    sizeLabel: z.string().trim().max(24).optional().or(z.literal("")),
    altLabel: z.string().trim().max(24).optional().or(z.literal("")),
    columns: z.array(z.string().trim().min(1).max(40)).min(1, "Add at least one measurement").max(12),
    rows: z
      .array(
        z.object({
          size: z.string().trim().min(1, "Every size needs a name").max(40),
          alt: z.string().trim().max(40).optional().or(z.literal("")),
          values: z.array(z.string().trim().max(20)),
        }),
      )
      .min(1, "Add at least one size")
      .max(60),
    note: z.string().trim().max(500).optional().or(z.literal("")),
  })
  .refine((g) => g.rows.every((r) => r.values.length === g.columns.length), {
    message: "Every size must have a value for each measurement",
  });

export const customerFieldSchema = z.object({
  id: z.string().trim().regex(/^[a-z0-9_-]{1,40}$/, "Field id must use lowercase letters, numbers, _ or -"),
  label: z.string().trim().min(1).max(100),
  type: z.enum(["text", "textarea", "number", "select", "checkbox", "date"]),
  required: z.boolean().default(false),
  placeholder: z.string().trim().max(160).optional().or(z.literal("")),
  helpText: z.string().trim().max(300).optional().or(z.literal("")),
  options: z.array(z.string().trim().min(1).max(100)).max(50).optional(),
}).superRefine((field, ctx) => {
  if (field.type === "select" && (!field.options || field.options.length === 0)) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["options"], message: "Select fields need at least one option" });
  }
  if (field.type !== "select" && field.options?.length) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["options"], message: "Options are only valid for select fields" });
  }
});

const customerFieldsSchema = z.array(customerFieldSchema).max(20).superRefine((fields, ctx) => {
  const seen = new Set();
  fields.forEach((field, index) => {
    if (seen.has(field.id)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: [index, "id"], message: "Customer field ids must be unique" });
    }
    seen.add(field.id);
  });
});

// Keep answers constrained by the product's current definition at the API
// boundary. The resulting object is stored as a snapshot on the request,
// invoice, and order, so later edits to the product form cannot rewrite
// historical customer instructions.
export function validateCustomerFieldAnswers(fields, answers) {
  const definitions = Array.isArray(fields) ? fields : [];
  const source = answers && typeof answers === "object" && !Array.isArray(answers) ? answers : {};
  const clean = {};
  for (const field of definitions) {
    const value = source[field.id];
    const empty = value == null || value === "" || (Array.isArray(value) && value.length === 0);
    if (field.required && (empty || (field.type === "checkbox" && value !== true))) {
      return { ok: false, error: `${field.label} is required` };
    }
    if (empty) continue;
    if (["text", "textarea", "date"].includes(field.type)) {
      if (typeof value !== "string" || value.length > 5000) return { ok: false, error: `${field.label} is invalid` };
      clean[field.id] = value.trim();
    } else if (field.type === "number") {
      const number = Number(value);
      if (!Number.isFinite(number) || String(value).length > 40) return { ok: false, error: `${field.label} must be a valid number` };
      clean[field.id] = number;
    } else if (field.type === "checkbox") {
      if (typeof value !== "boolean") return { ok: false, error: `${field.label} must be checked or unchecked` };
      clean[field.id] = value;
    } else if (field.type === "select") {
      if (typeof value !== "string" || !field.options?.includes(value)) return { ok: false, error: `${field.label} has an invalid option` };
      clean[field.id] = value;
    }
  }
  return { ok: true, data: clean };
}

export function customerFieldKey(answers) {
  const source = answers && typeof answers === "object" && !Array.isArray(answers) ? answers : {};
  return JSON.stringify(Object.keys(source).sort().reduce((result, key) => {
    result[key] = source[key];
    return result;
  }, {}));
}

const productVariantFieldsSchema = z.object({
  options: z.record(z.string().trim().min(1).max(50), z.string().trim().min(1).max(100))
    .refine((options) => Object.keys(options).length > 0, "At least one option is required")
    .refine((options) => Object.keys(options).length <= 5, "Up to 5 option dimensions are allowed"),
  sku: optionalTrimmedText(z.string().trim().min(1).max(100)),
  price: z.coerce.number().positive().nullable().optional(),
  stock: z.coerce.number().int().min(0).nullable().optional(),
  isActive: z.boolean().optional(),
});

const productFieldsSchema = z.object({
  name: z.string().trim().min(1, "Product name is required").max(200),
  slug: slugField,
  sku: optionalTrimmedText(z.string().trim().min(1).max(100)),
  description: optionalTrimmedText(z.string().trim().min(1).max(5000)),
  // Structured sizing chart (see products.sizeGuide). Every row must
  // carry exactly one value per column. null clears it.
  sizeGuide: sizeGuideSchema.nullable().optional(),
  // Invoice-required products store zero as a sentinel. Their amount is
  // supplied later on the invoice and is never used by normal checkout.
  price: z.coerce.number().min(0, "Price cannot be negative"),
  // What the vendor paid - optional, internal only. null/"" clears it.
  costPrice: z.coerce.number().min(0).nullable().optional(),
  // Wholesale / bundle pricing. Each: every whole group of `bundleQty`
  // units is charged `unitPrice` each, leftovers at the normal price -
  // see computeWholesalePrice in lib/pricing.js. [] or null clears.
  priceTiers: z
    .array(
      z.object({
        bundleQty: z.coerce.number().int().min(2, "A bundle is 2 units or more"),
        unitPrice: z.coerce.number().positive(),
      }),
    )
    .max(6)
    .nullable()
    .optional(),
  // Actually reduces what's charged - see lib/pricing.js's
  // getEffectivePrice, not just a display trick. null/"" clears it (same
  // as the other nullable PATCH fields).
  discountPercent: z.coerce.number().min(1).max(99).nullable().optional(),
  productType: z.enum(["physical", "digital"]).default("physical"),
  saleMode: z.enum(["fixed_price", "invoice_required"]).default("fixed_price"),
  customerFields: customerFieldsSchema.default([]),
  condition: z.enum(["new", "used", "fairly_used"]).default("new"),
  stock: z.coerce.number().int().min(0).optional(),
  // Perishable goods: a single "use by" date for the product. "" / null
  // clears it. Internal - stripped from public/storefront reads.
  expiryDate: z
    .string()
    .trim()
    .regex(/^\d{4}-\d{2}-\d{2}$/, "Use YYYY-MM-DD")
    .nullable()
    .optional()
    .or(z.literal("")),
  categoryId: optionalTrimmedText(z.string().trim().min(1)).nullable().optional(),
  images: z.array(z.string().trim().url()).max(10, "Up to 10 photos and video combined").optional(),
  // One short clip, see products.videoUrl. "" clears it, same as the
  // other nullable PATCH fields.
  videoUrl: z.string().trim().url().optional().or(z.literal("")),
  isActive: z.boolean().optional(),
  allowStandardVariant: z.boolean().optional(),
});

export const createProductSchema = productFieldsSchema.extend({
  // Multi-branch stores only: opening stock per branch, set on the
  // create form. Ignored on single-branch stores (the plain `stock`
  // field covers those). Unknown branch ids are dropped server-side.
  branchStock: z
    .array(z.object({ branchId: z.string().trim().min(1), stock: z.coerce.number().int().min(0) }))
    .max(50)
    .optional(),
  variants: z.array(productVariantFieldsSchema).max(200).optional(),
}).superRefine((data, ctx) => {
  if (data.saleMode === "fixed_price" && data.price <= 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["price"], message: "Price must be greater than 0 for fixed-price products" });
  }
  if (data.saleMode === "invoice_required" && data.price !== 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["price"], message: "Invoice-required products cannot have a fixed price" });
  }
  if (data.saleMode === "invoice_required" && (data.discountPercent != null || data.priceTiers?.length)) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["saleMode"], message: "Invoice-required products cannot use fixed-price discounts or tiers" });
  }
});
// PATCH must not inherit the create schema's defaults. Zod defaults still run
// through optional fields produced by partial(), which previously made a
// category-only update silently reset saleMode, productType, condition, and
// customerFields.
const updateProductFieldsSchema = productFieldsSchema.extend({
  productType: z.enum(["physical", "digital"]),
  saleMode: z.enum(["fixed_price", "invoice_required"]),
  customerFields: customerFieldsSchema,
  condition: z.enum(["new", "used", "fairly_used"]),
});

export const updateProductSchema = updateProductFieldsSchema.partial().superRefine((data, ctx) => {
  if (data.saleMode === "fixed_price" && data.price != null && data.price <= 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["price"], message: "Price must be greater than 0 for fixed-price products" });
  }
  if (data.saleMode === "invoice_required" && data.price != null && data.price !== 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["price"], message: "Invoice-required products cannot have a fixed price" });
  }
});

// One row of a bulk-import CSV. No `slug` (derived from name server-side,
// same as single-product creation's auto-slugify) and no variant columns
// - a spreadsheet can't reasonably express "N variants with options and
// per-variant stock" as flat cells, so bulk-created products start
// variant-less and images-less; a vendor adds either afterward by
// editing the product like normal.
//
// Every cell arrives as a string (blank -> ""), so optional text columns
// map "" -> undefined rather than tripping .min(1), and the two enum
// columns are lower-cased / de-spaced so "Physical" or "fairly used"
// still land. A blank stock cell means "not tracked", not 0.
const bulkOptText = z.preprocess(
  (v) => (typeof v === "string" && v.trim() === "" ? undefined : v),
  z.string().trim().min(1).optional(),
);
const bulkEnum = (values, fallback) =>
  z.preprocess((v) => {
    if (typeof v !== "string") return v;
    const s = v.trim().toLowerCase().replace(/[\s-]+/g, "_");
    return s === "" ? undefined : s;
  }, z.enum(values).default(fallback));

export const bulkProductRowSchema = z.object({
  name: z.string().trim().min(1, "Product name is required").max(200),
  price: z.coerce.number().positive("Price must be greater than 0"),
  costPrice: z.preprocess((v) => (v === "" || v == null ? undefined : v), z.coerce.number().min(0).optional()),
  sku: z.preprocess(
    (v) => (typeof v === "string" && v.trim() === "" ? undefined : v),
    z.string().trim().min(1).max(100).optional(),
  ),
  description: z.preprocess(
    (v) => (typeof v === "string" && v.trim() === "" ? undefined : v),
    z.string().trim().min(1).max(5000).optional(),
  ),
  productType: bulkEnum(["physical", "digital"], "physical"),
  condition: bulkEnum(["new", "used", "fairly_used"], "new"),
  stock: z.preprocess(
    (v) => (v === "" || v == null ? undefined : v),
    z.coerce.number().int().min(0).optional(),
  ),
  expiryDate: z.preprocess(
    (v) => (v === "" || v == null ? undefined : v),
    z.string().trim().regex(/^\d{4}-\d{2}-\d{2}$/, "Use YYYY-MM-DD").optional(),
  ),
  categoryName: bulkOptText,
  // The in-app grid passes a category id directly (a picked option),
  // rather than a name to fuzzy-match like a CSV row. Wins over categoryName.
  categoryId: bulkOptText,
});

export const createVariantSchema = productVariantFieldsSchema.extend({
  // null clears a previously set override back to "use the product price"
  // - price/stock aren't collected when a variant is first added, only
  // when it's edited afterward (see VariantsManager in the edit page).
});

export const updateVariantSchema = createVariantSchema.partial();

export const createReviewSchema = z.object({
  rating: z.coerce.number().int().min(1).max(5),
  comment: optionalTrimmedText(z.string().trim().min(1, "Write a review or leave the comment blank").max(2000)),
  // A URL from /api/v1/storefront/reviews/upload - the route only ever
  // returns URLs on our own upload bucket, re-checked server-side before
  // the review is saved.
  imageUrl: optionalTrimmedText(z.string().trim().url("Invalid image URL").max(2000)),
});

// quantity is capped well above any real order - it exists to stop an
// absurd value (overflowing the line-total math, or a junk cart) rather
// than to be a business rule. Real availability is enforced against stock.
export const addCartItemSchema = z.object({
  productId: z.string().trim().min(1, "Product is required"),
  variantId: optionalId("Select a product option"),
  quantity: z.coerce.number().int().min(1).max(100000).default(1),
  customerFields: z.record(z.string(), z.unknown()).optional().default({}),
});

export const invoiceRequestItemSchema = z.object({
  productId: z.string().trim().min(1),
  variantId: optionalId("Select a product option").nullable().optional(),
  quantity: z.coerce.number().int().min(1).max(100000),
  customerFields: z.record(z.string(), z.unknown()).optional().default({}),
});

function rejectDuplicateInvoiceItems(data, ctx) {
  const seen = new Set();
  data.items.forEach((item, index) => {
    const key = `${item.productId}:${item.variantId || ""}`;
    if (seen.has(key)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["items", index], message: "The same product option can only appear once" });
    }
    seen.add(key);
  });
}

export const createInvoiceRequestSchema = z.object({
  branchId: optionalId("Select a branch"),
  guestEmail: z.string().trim().toLowerCase().email("Valid email is required"),
  buyerName: z.string().trim().min(1, "Customer name is required").max(150),
  buyerPhone: z.string().trim().min(5, "Customer phone is required").max(30),
  note: z.string().trim().max(2000).optional().or(z.literal("")),
  items: z.array(invoiceRequestItemSchema).min(1, "Add at least one product").max(100),
}).superRefine(rejectDuplicateInvoiceItems);

export const createInvoiceSchema = z.object({
  requestId: z.string().trim().min(1),
  guestEmail: optionalEmail(),
  plan: z.enum(["full", "deposit"]).default("full"),
  expiresAt: z.coerce.date().optional(),
  items: z.array(z.object({
    productId: z.string().trim().min(1),
    variantId: optionalId("Select a product option").nullable().optional(),
    quantity: z.coerce.number().int().min(1).max(100000),
    unitPrice: z.coerce.number().positive(),
    customerFields: z.record(z.string(), z.unknown()).optional().default({}),
  })).min(1).max(100),
  note: z.string().trim().max(2000).optional().or(z.literal("")),
}).superRefine(rejectDuplicateInvoiceItems);

export const updateCartItemSchema = z.object({
  quantity: z.coerce.number().int().min(1, "Quantity must be at least 1").max(100000),
});

// Customer self-signup at a specific store (resolved from the request's
// Host header, not submitted by the client) - no terms/store fields,
// unlike vendorSignupSchema, since the customer isn't creating a store.
export const customerSignupSchema = z.object({
  firstName: z.string().trim().min(1, "First name is required"),
  lastName: z.string().trim().min(1, "Last name is required"),
  email: signupEmail,
  password: z.string().min(8, "Password must be at least 8 characters"),
  acceptMarketing: z.boolean().optional(),
});

const addressFields = {
  fullName: z.string().trim().min(1, "Full name is required").max(150),
  phone: z.string().trim().min(1, "Phone number is required").max(30),
  line1: z.string().trim().min(1, "Address line is required").max(300),
  line2: z.string().trim().max(200).optional().or(z.literal("")),
  city: z.string().trim().min(1, "City is required").max(120),
  state: z.string().trim().min(1, "State is required").max(120),
  country: z.string().trim().min(1).max(120).optional(),
  isDefault: z.boolean().optional(),
};
export const createAddressSchema = z.object(addressFields);
export const updateAddressSchema = z.object(addressFields).partial();

export const checkoutSchema = z.object({
  guestEmail: optionalEmail(),
  addressId: optionalId("Select a delivery address"),
  shippingAddress: z.object(addressFields).optional(),
  note: z.string().trim().max(500).optional().or(z.literal("")),
});

// A vendor manually recording an in-person/phone/cash sale that never
// went through storefront checkout - no address/shipping fields (the
// vendor already handed the item over or is arranging delivery
// themselves), and the buyer doesn't need an account or even an email.
export const createOfflineOrderSchema = z.object({
  // Idempotency: the record-a-sale form mints one id per submit, so a
  // double-click or a retry after a timeout returns the order already
  // created instead of recording the sale (and decrementing stock) twice.
  // Same mechanism as posSaleSchema. Optional so an older client that
  // doesn't send one still works, just without the protection.
  idempotencyKey: z.string().trim().min(8).max(64).optional(),
  buyerName: z.string().trim().min(1, "Customer name is required").max(150),
  buyerEmail: optionalEmail("Invalid email address").or(z.literal("")),
  buyerPhone: z.string().trim().max(20).optional().or(z.literal("")),
  note: z.string().trim().max(500).optional().or(z.literal("")),
  delivered: z.boolean().default(false),
  // How the money came in, so a recorded sale carries its payment method
  // like a till sale does. "card" = a POS machine; paymentProvider names
  // it (Moniepoint / Opay / ...).
  paymentMethod: z.enum(["cash", "card", "transfer"]).default("cash"),
  paymentProvider: z.string().trim().max(40).optional().or(z.literal("")),
  // Required only once a store has more than one branch - a
  // branch-scoped staff member's own branch is used regardless of what's
  // submitted here (see the route).
  branchId: optionalId("Select a branch"),
  items: z
    .array(
      z.object({
        productId: z.string().trim().min(1),
        variantId: optionalId("Select a product option"),
        quantity: z.number().int().positive(),
      }),
    )
    .min(1, "At least one item is required"),
});

export const requestRefundSchema = z.object({
  reason: z.string().trim().min(1, "A reason is required").max(2000),
});

// ---------- POINT OF SALE ----------
// All money fields below are in naira (what the sell screen collects);
// the routes convert to integer kobo at the edge via lib/money.js.

export const createRegisterSchema = z.object({
  name: z.string().trim().min(1, "Give the register a name").max(60),
  branchId: z.string().trim().min(1, "Pick a branch"),
});
export const updateRegisterSchema = z.object({
  name: z.string().trim().min(1, "Give the register a name").max(60).optional(),
  isActive: z.boolean().optional(),
});

// POS drawer/tender columns are PostgreSQL integers in kobo. Stay below
// the signed 32-bit ceiling after naira-to-kobo conversion.
const MAX_POS_NAIRA = 20_000_000;
const CASH_DENOMINATIONS = new Set(["5", "10", "20", "50", "100", "200", "500", "1000"]);

export const openSessionSchema = z.object({
  registerId: z.string().trim().min(1),
  openingFloat: z.coerce.number().min(0).max(MAX_POS_NAIRA).default(0),
});

export const closeSessionSchema = z
  .object({
    // Plain total (naira). Optional when a note breakdown or a forced
    // close is sent instead.
    countedCash: z.coerce.number().min(0).max(MAX_POS_NAIRA).optional(),
    // Note-by-note count: { "1000": 12, "500": 3, ... } (naira denom -> qty).
    countBreakdown: z.record(z.string(), z.coerce.number().int().min(0).max(100_000)).optional(),
    // The cashier couldn't count the drawer (a bug, a rush). The system's
    // expected figure is used and the shift is flagged for the owner.
    forced: z.boolean().optional(),
    forcedReason: z.string().trim().max(300).optional(),
    // How many sales were still unsynced on a device at close time.
    pendingSyncCount: z.coerce.number().int().min(0).max(999).optional(),
  })
  .refine((d) => d.forced || d.countedCash != null || d.countBreakdown, {
    message: "Enter the counted cash",
    path: ["countedCash"],
  })
  .refine((d) => !d.forced || (d.forcedReason && d.forcedReason.length > 0), {
    message: "Say why the drawer couldn't be counted",
    path: ["forcedReason"],
  })
  .refine((d) => !d.countBreakdown || Object.keys(d.countBreakdown).every((key) => CASH_DENOMINATIONS.has(key)), {
    message: "Cash count contains an unsupported denomination",
    path: ["countBreakdown"],
  })
  .refine((d) => !d.countBreakdown || Object.entries(d.countBreakdown).reduce((sum, [denom, quantity]) => sum + Number(denom) * quantity, 0) <= MAX_POS_NAIRA, {
    message: "Counted cash is too large",
    path: ["countBreakdown"],
  });

// Owner sign-off on a flagged register close.
export const reviewSessionSchema = z.object({
  note: z.string().trim().max(500).optional(),
});

export const cashMovementSchema = z.object({
  kind: z.enum(["paid_in", "paid_out", "drop"]),
  amount: z.coerce.number().positive("Enter an amount").max(MAX_POS_NAIRA),
  reason: z.string().trim().min(1, "A reason is required").max(200),
  // One value per button press, reused on every retry - the server
  // dedupes on it so a flaky network can't record the same payout twice.
  clientRef: z.string().trim().min(8).max(64).optional(),
});

const posLineSchema = z.object({
  productId: z.string().trim().min(1),
  variantId: optionalId("Select a product option"),
  quantity: z.number().int().positive().max(100_000, "Quantity is too large"),
  // A cashier-typed unit price (naira) replacing the catalogue price.
  unitPrice: z.coerce.number().min(0).optional(),
  // Per-line markdown in naira, >= 0.
  lineDiscount: z.coerce.number().min(0).max(MAX_POS_NAIRA).optional(),
  // Price snapshot shown at the till before line discount. Used only for
  // a delayed offline replay; live sales remain server-priced.
  capturedLineTotal: z.coerce.number().min(0).max(MAX_POS_NAIRA).optional(),
});

const posTenderSchema = z.object({
  method: z.enum(["cash", "card", "transfer"]),
  // POS terminal / provider for a card payment (e.g. "Moniepoint",
  // "Opay"). Free text so a new machine doesn't need a code change.
  provider: z.string().trim().max(40).optional().or(z.literal("")),
  amount: z.coerce.number().positive().max(MAX_POS_NAIRA),
  changeGiven: z.coerce.number().min(0).max(MAX_POS_NAIRA).optional(),
  reference: z.string().trim().max(120).optional().or(z.literal("")),
});

export const posSaleSchema = z.object({
  sessionId: z.string().trim().min(1),
  // Idempotency: the sell screen mints one id per ring-up; a retry with
  // the same key returns the already-created order instead of a second.
  idempotencyKey: z.string().trim().min(8).max(64),
  // The sell screen generates the order number up front (so a receipt
  // printed at the counter matches the row, even for a sale that only
  // syncs later). Server falls back to its own if absent/taken. Accepts
  // null as well as absent - an offline payload can serialise it as null,
  // and rejecting that stranded queued sales forever.
  orderNumber: z.string().trim().regex(/^ORD-[A-Z0-9]{6,10}$/).nullish(),
  heldSaleId: optionalId("Select a held sale"),
  // ISO timestamp of when the sale was actually rung up - set for a sale
  // that was queued offline and is syncing now. Server bounds it.
  soldAt: z.string().trim().datetime().nullish(),
  offlineReplay: z.boolean().optional(),
  buyerName: z.string().trim().max(150).optional().or(z.literal("")),
  buyerEmail: optionalEmail("Invalid email address").or(z.literal("")),
  buyerPhone: z.string().trim().max(20).optional().or(z.literal("")),
  note: z.string().trim().max(500).optional().or(z.literal("")),
  discountAmount: z.coerce.number().min(0).max(MAX_POS_NAIRA).optional(),
  discountReason: z.string().trim().max(200).optional().or(z.literal("")),
  items: z.array(posLineSchema).min(1, "Add at least one item").max(200, "A sale can contain at most 200 lines")
    .refine((items) => new Set(items.map((item) => `${item.productId}:${item.variantId || ""}`)).size === items.length, "Combine duplicate product lines"),
  tenders: z.array(posTenderSchema).min(1, "Take at least one payment").max(10, "A sale can contain at most 10 payments"),
});

export const holdSaleSchema = z.object({
  sessionId: z.string().trim().min(1),
  label: z.string().trim().max(80).optional().or(z.literal("")),
  cart: z.array(z.record(z.string(), z.any())).min(1, "Nothing to hold").max(200, "A held sale can contain at most 200 lines"),
  customer: z.record(z.string(), z.any()).nullable().optional(),
});

export const posReturnSchema = z.object({
  sessionId: z.string().trim().min(1),
  idempotencyKey: z.string().trim().min(8).max(64),
  originalOrderId: z.string().trim().min(1),
  refundMethod: z.enum(["cash", "transfer", "card"]),
  reference: z.string().trim().max(120).optional().or(z.literal("")),
  items: z
    .array(z.object({ orderItemId: z.string().trim().min(1), quantity: z.number().int().positive().max(100_000) }))
    .min(1, "Pick at least one line to return")
    .max(200, "A return can contain at most 200 lines")
    .refine((items) => new Set(items.map((item) => item.orderItemId)).size === items.length, "Each order line can only appear once"),
});

export const updateOrderStatusSchema = z.object({
  status: z.enum(["processing", "shipped", "delivered", "cancelled"]).optional(),
  refundDecision: z.enum(["approved", "rejected"]).optional(),
  reviewNote: optionalTrimmedText(z.string().trim().min(1, "Add a review note").max(2000)),
  // Only meaningful on an order with shippingFeeTBD true and no
  // shippingFeeConfirmedAt yet - the vendor recording the real delivery
  // fee once known, for their own records (never re-charged, see
  // PATCH .../orders/[id]).
  shippingFee: z.coerce.number().min(0).optional(),
});

// NIN (National Identification Number) is always exactly 11 digits.
// The client encrypts the raw NIN with the platform's public key before
// it's ever sent (see lib/ninClient.js) - by the time it reaches this
// schema it's RSA-OAEP ciphertext, base64-encoded, not the digits
// themselves. The 11-digit shape is checked after the route decrypts it
// server-side, see lib/nin.js.
export const submitNinSchema = z.object({
  nin: z.string().trim().min(1, "Missing encrypted NIN").max(1000),
});

export const reviewVendorApprovalSchema = z.object({
  decision: z.enum(["approved", "rejected"]),
  reviewNote: z.string().trim().min(1).optional(),
});

export const suspendProductSchema = z.object({
  suspended: z.boolean(),
  reason: z.string().trim().min(1).optional(),
});

// The browser push services a real PushSubscription.endpoint can belong
// to. lib/push.js POSTs to this URL from the server, so an unrestricted
// value made every authenticated user able to aim a server-side request
// at any host they liked - internal services, cloud metadata - by
// registering it and then triggering any notification for themselves.
const PUSH_ENDPOINT_HOSTS = [
  "fcm.googleapis.com",
  "android.googleapis.com",
  "push.services.mozilla.com",
  "web.push.apple.com",
  "push.apple.com",
  "notify.windows.com",
];

export function isAllowedPushEndpoint(value) {
  let url;
  try {
    url = new URL(value);
  } catch {
    return false;
  }
  if (url.protocol !== "https:") return false;
  const hostname = url.hostname.toLowerCase();
  return PUSH_ENDPOINT_HOSTS.some((host) => hostname === host || hostname.endsWith(`.${host}`));
}

export const pushSubscribeSchema = z.object({
  endpoint: z
    .string()
    .trim()
    .url()
    .max(1000)
    .refine(isAllowedPushEndpoint, "That is not a recognised push service endpoint"),
  keys: z.object({
    p256dh: z.string().trim().min(1).max(200),
    auth: z.string().trim().min(1).max(200),
  }),
});

export const createBranchSchema = z.object({
  name: z.string().trim().min(1, "Branch name is required").max(150),
  address: z.string().trim().max(300).optional().or(z.literal("")),
});

export const updateBranchSchema = createBranchSchema.partial();

export const inviteStaffSchema = z.object({
  firstName: z.string().trim().min(1, "First name is required").max(100),
  lastName: z.string().trim().min(1, "Last name is required").max(100),
  email: z.string().trim().toLowerCase().email("Valid email is required"),
  // Required only once a store has more than one branch (see the route -
  // a single-branch store auto-assigns its one branch, nothing to pick).
  branchId: z.string().trim().min(1).optional(),
});

// Reassign an existing staff member to a different branch. null clears
// the scope (the member can then see/manage every branch).
export const updateStaffSchema = z.object({
  branchId: z.string().trim().min(1).nullable(),
});

export const inviteTeamMemberSchema = z.object({
  firstName: z.string().trim().min(1, "First name is required").max(100),
  lastName: z.string().trim().min(1, "Last name is required").max(100),
  email: z.string().trim().toLowerCase().email("Valid email is required"),
  role: z.enum(["admin", "p_staff"]),
});

// Add an email or domain to the signup block-list. The route works out
// `kind` (an "@" -> email, else domain) and canonicalises the value.
export const blockedEmailSchema = z.object({
  value: z.string().trim().toLowerCase().min(3, "Enter an email or domain").max(200),
  reason: z.string().trim().max(200).optional().or(z.literal("")),
});

// Ban a customer and/or a device from the super-admin review UI.
export const createBanSchema = z
  .object({
    customerId: z.string().trim().min(1).optional(),
    deviceId: z.string().trim().min(1).max(64).optional(),
    fingerprint: z.string().trim().max(32).optional(),
    // Ban the device the given customer signed up from, too.
    banSignupDevice: z.boolean().optional(),
    reason: z.string().trim().min(1, "A reason is required").max(300),
  })
  .refine((d) => d.customerId || d.deviceId, { message: "Nothing to ban", path: ["customerId"] });

export const updateTeamMemberSchema = z.object({
  role: z.enum(["admin", "p_staff"]).optional(),
  // Suspend/reactivate short of outright removal - blocks login
  // immediately (see users.isBanned in getUser/lib/auth.js) without
  // losing the account or its activity history.
  isBanned: z.boolean().optional(),
}).refine((data) => data.role !== undefined || data.isBanned !== undefined, {
  message: "Nothing to update",
});

export const sendNotificationSchema = z.object({
  title: z.string().trim().min(1, "Title is required").max(100),
  body: z.string().trim().min(1, "Message is required").max(500),
  url: z.string().trim().max(300).optional(),
  channel: z.enum(["push", "email", "both"]),
  target: z.enum(["single", "all_vendors"]),
  userId: z.string().trim().min(1).optional(),
}).refine((data) => data.target !== "single" || !!data.userId, {
  message: "Pick a vendor to send to",
  path: ["userId"],
});
