...
const productFieldsSchema = z.object({
  ...
  // "Was" price for a struck-through discount display, see
  // products.compareAtPrice in lib/db/schema.js. null/"" clears it
  // (handled the same way as the other nullable PATCH fields).
...
const compareAtPriceRefinement = (data) => data.compareAtPrice == null || data.price == null || data.compareAtPrice > data.price;
const compareAtPriceRefinementOpts = { message: "Compare-at price must be higher than the actual price", path: ["compareAtPrice"] };

export const createProductSchema = productFieldsSchema.refine(compareAtPriceRefinement, compareAtPriceRefinementOpts);
export const updateProductSchema = productFieldsSchema.partial().refine(compareAtPriceRefinement, compareAtPriceRefinementOpts);
