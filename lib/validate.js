...
const productFieldsSchema = z.object({
  ...
  // Wholesale / quantity price breaks. Each: buy `minQty`+ and the unit
  // price is `unitPrice`. Sorted + de-duped server-side isn't required -
  // tieredUnitPrice just picks the best matching tier. [] or null clears.
  priceTiers: z
    .array(
      z.object({
        minQty: z.coerce.number().int().min(2, "A bulk tier starts at 2 or more"),
