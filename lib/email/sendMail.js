...
export async function sendMail({ to, subject, html, from, fromName, replyTo, attachments, brand, preheader }) {
  ...
  await sendViaSendPulse({
    html: Buffer.from(renderEmail({ content: html, brand, preheader: preheader || subject }), "utf-8").toString("base64"),
    subject,
