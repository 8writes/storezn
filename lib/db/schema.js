...
// ---------- ENUMS ----------
export const roleEnum = pgEnum("role", ["customer", "vendor", "super_admin"]);
...
export const users = pgTable(
  ...
  {
    id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
    // A customer belongs to exactly one store (mirrors school-app's
    // students-belong-to-one-school shape). Null for vendor/super_admin -
    // a vendor's store relationship is via stores.ownerId instead, since
    // that supports owning more than one store.
