...
import { relations, sql, isNull, isNotNull } from "drizzle-orm";
import { productNameKeyExpression } from "../productName.js";
...
export const productSaleModeEnum = pgEnum("product_sale_mode", ["fixed_price", "invoice_required"]);
...
export const paymentStatusEnum = pgEnum("payment_status", ["pending", "partially_paid", "paid", "failed"]);
export const invoiceRequestStatusEnum = pgEnum("invoice_request_status", ["new", "reviewing", "quoted", "converted", "cancelled", "expired"]);
export const invoiceStatusEnum = pgEnum("invoice_status", ["draft", "sent", "partially_paid", "paid", "expired", "void", "cancelled"]);
export const invoicePlanEnum = pgEnum("invoice_plan", ["full", "deposit"]);
export const invoicePaymentKindEnum = pgEnum("invoice_payment_kind", ["full", "deposit", "balance", "external"]);
...
export const stores = pgTable(
  "stores",
  {
    ...
    // when set, falling back to an auto-generated one otherwise. Also
    // shown as the tagline under the store name on the storefront home
    // page when showDescription is on.
    description: text("description"),
    // Whether `description` is shown as the storefront home tagline (the
    // meta-description use above is unaffected either way). Defaults on -
    // a store that has bothered to write one usually wants it seen. See
    // STOREFRONT-DESCRIPTION-MIGRATION.sql.
    ...
    // Optional one-month introductory discount from the Storezn+ price.
    // subscriptionPriceOverride stores the resulting amount so the value
    // charged by the dedicated Paystack plan remains explicit and auditable;
    // the subscription.create webhook consumes it and restores normal price.
    subscriptionDiscountPercent: real("subscription_discount_percent"),
...
export const users = pgTable(
  "users",
  {
    ...
    // see lib/emailNormalize.js, same as customers.normalizedEmail. Backs
    // the real uniqueness check at vendor self-signup so one mailbox can't
    // become several vendor accounts (and several stores) through dot
    // aliases. Null for rows that predate this column and for
    // admin-created accounts, which is why the unique index below is
    // partial.
    normalizedEmail: text("normalized_email"),
    phone: text("phone"),
    passwordHash: text("password_hash").notNull(),
    // Set whenever this account's password changes (reset link, or the
    // change-password form). JWTs are stateless and live 30 days, so
    // without this a password change did not evict anyone already holding
    // a token - the whole point of changing it after a compromise. getUser
    // (lib/auth.js) rejects any token issued before this moment.
  ...
  (table) => ({
    ...
    // Partial, like customers' equivalent: only live rows that actually
    // have a normalized form participate, so pre-existing and
    // admin-created accounts are unaffected.
    normalizedEmailIdx: uniqueIndex("uq_users_normalized_email")
...
// Persistent server-side error log for super-admin diagnostics. This is
// intentionally append-only from application code; admins can mark rows
// resolved from the dashboard without losing the original failure data.
export const appErrorLogs = pgTable(
  ...
  (table) => ({
    createdAtIdx: index("idx_app_error_logs_created_at").on(table.createdAt),
    levelIdx: index("idx_app_error_logs_level").on(table.level),
    routeIdx: index("idx_app_error_logs_route").on(table.route),
    resolvedIdx: index("idx_app_error_logs_resolved").on(table.resolved),
  }),
);

// Lightweight API request monitoring. Stores endpoint/status/duration and
// the short public error message returned by a failed endpoint, but never
// request bodies, auth headers, or complete response payloads.
export const apiRequestLogs = pgTable(
  ...
  (table) => ({
    ...
    sourceIdx: index("idx_api_request_logs_source").on(table.source),
    routeIdx: index("idx_api_request_logs_route").on(table.route),
    statusIdx: index("idx_api_request_logs_status").on(table.statusCode),
  }),
);

// Shared sliding-window counters for public/authenticated abuse guards.
// Unlike the old process-local map, this survives multiple app workers and
// deploys. Expired rows are pruned opportunistically by lib/rateLimit.js.
...
export const storeActivityLogs = pgTable(
  ...
  (table) => ({
    ...
    targetCreatedIdx: index("idx_store_activity_target_created").on(table.storeId, table.targetType, table.targetId, table.createdAt),
    flaggedIdx: index("idx_store_activity_flagged").on(table.storeId, table.flaggedAt, table.reviewedAt),
  }),
);
...
export const tokens = pgTable(
  ...
  (table) => ({
    ...
    expiresAtIdx: index("idx_tokens_expires_at").on(table.expiresAt),
    usedAtIdx: index("idx_tokens_used_at").on(table.usedAt),
  }),
);
...
export const products = pgTable(
  "products",
  {
    ...
    // Fixed-price products use normal checkout/POS. Invoice-required
    // products can only enter the quote/request flow; their stored price is
    // never used as a customer charge.
    saleMode: productSaleModeEnum("sale_mode").notNull().default("fixed_price"),
    // Vendor-defined fields the buyer must answer for this product, e.g.
    // measurements, engraving text, or event details. Answers are
    // snapshotted on request/cart/order rows and never read live later.
    ...
    // One short clip per product (see lib/mediaValidation.js) - kept separate from images
  ...
  (table) => ({
    ...
    storeNameNormalizedUnique: uniqueIndex("uq_products_store_name_normalized").on(
      table.storeId,
      productNameKeyExpression(table.name),
    ),
...
export const platformSettings = pgTable("platform_settings", {
  ...
  // Platform-wide introductory offer for new Storezn+ subscriptions.
  // Null disables it; the first charge is discounted and renewals use
  // plusMonthlyPrice through a temporary per-store Paystack plan.
  plusIntroDiscountPercent: real("plus_intro_discount_percent"),
...
export const cartItems = pgTable(
  "cart_items",
  {
    ...
    variantId: text("variant_id").references(() => productVariants.id),
    // Answers to vendor-defined customer fields. The same product/variant
    // can appear more than once when buyers provide different answers.
    customerFields: jsonb("customer_fields").notNull().default({}),
  ...
  (table) => ({
    ...
    cartProductVariantUnique: uniqueIndex("uq_cart_items_cart_product_variant").on(table.cartId, table.productId, table.variantId, table.customizationKey),
    cartBaseProductUnique: uniqueIndex("uq_cart_items_cart_product_base").on(table.cartId, table.productId, table.customizationKey).where(isNull(table.variantId)),
    cartVariantUnique: uniqueIndex("uq_cart_items_cart_variant").on(table.cartId, table.productId, table.variantId, table.customizationKey).where(isNotNull(table.variantId)),
  }),
);
...
export const orders = pgTable(
  "orders",
  {
    ...
    paymentAuthorizationExpiresAt: timestamp("payment_authorization_expires_at"),
    transactionReference: text("transaction_reference").unique(),
    paidAt: timestamp("paid_at"),
    // Invoice orders can receive more than one payment. These cached values
    // are updated from the invoice payment ledger inside the same database
    // transaction as each verified payment.
  ...
  (table) => ({
    ...
    posSessionIdIdx: index("idx_orders_pos_session_id").on(table.posSessionId),
    originalOrderIdIdx: index("idx_orders_original_order_id").on(table.originalOrderId),
...
// Short-lived payment attempts created when a shopper clicks "Pay now".
// This keeps mutable/pre-payment state out of orders: the cart is
// snapshotted here, stock is reserved here, and only a confirmed Paystack
// payment promotes the attempt into an immutable order/order_items row.
export const checkoutAttempts = pgTable(
  ...
  (table) => ({
    storeIdIdx: index("idx_checkout_attempts_store_id").on(table.storeId),
    cartIdIdx: index("idx_checkout_attempts_cart_id").on(table.cartId),
    paymentStatusIdx: index("idx_checkout_attempts_payment_status").on(table.paymentStatus),
    cartPendingAttemptUnique: uniqueIndex("uq_checkout_attempts_cart_pending")
      .on(table.cartId)
      .where(sql`${table.paymentStatus} = 'pending'`),
  }),
);
...
export const checkoutAttemptsRelations = relations(checkoutAttempts, ({ one }) => ({
  store: one(stores, { fields: [checkoutAttempts.storeId], references: [stores.id] }),
  branch: one(branches, { fields: [checkoutAttempts.branchId], references: [branches.id] }),
  user: one(customers, { fields: [checkoutAttempts.userId], references: [customers.id] }),
  cart: one(carts, { fields: [checkoutAttempts.cartId], references: [carts.id] }),
  order: one(orders, { fields: [checkoutAttempts.orderId], references: [orders.id] }),
}));

// ---------- INVOICE / QUOTE FLOW ----------

// A request contains no financial order yet. This is deliberate: vendors
// need to agree the price before the request can become a payable order.
export const invoiceRequests = pgTable(
  "invoice_requests",
  {
    ...
    buyerPhone: text("buyer_phone"),
    note: text("note"),
    status: invoiceRequestStatusEnum("status").notNull().default("new"),
  ...
  (table) => ({
    storeIdIdx: index("idx_invoice_requests_store_id").on(table.storeId),
    statusIdx: index("idx_invoice_requests_status").on(table.status),
    createdAtIdx: index("idx_invoice_requests_created_at").on(table.createdAt),
  }),
);
...
export const invoiceRequestItems = pgTable(
  ...
  {
    id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
    requestId: text("request_id").notNull().references(() => invoiceRequests.id),
    productId: text("product_id").notNull().references(() => products.id),
    variantId: text("variant_id").references(() => productVariants.id),
  ...
  (table) => ({
    requestIdIdx: index("idx_invoice_request_items_request_id").on(table.requestId),
  }),
);

// A sent invoice is the financial agreement. It owns the order once the
// vendor sends it, while each Paystack attempt/payment remains in the
// append-only invoice_payments ledger below.
export const invoices = pgTable(
  "invoices",
  {
    id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
    storeId: text("store_id").notNull().references(() => stores.id),
    requestId: text("request_id").references(() => invoiceRequests.id),
    orderId: text("order_id").references(() => orders.id),
    invoiceNumber: text("invoice_number").notNull().unique(),
    shareToken: text("share_token").notNull().unique(),
    status: invoiceStatusEnum("status").notNull().default("draft"),
    plan: invoicePlanEnum("plan").notNull().default("full"),
    totalAmount: real("total_amount").notNull(),
    amountPaid: real("amount_paid").notNull().default(0),
  ...
  (table) => ({
    storeIdIdx: index("idx_invoices_store_id").on(table.storeId),
    requestUnique: uniqueIndex("uq_invoices_request_id").on(table.requestId),
    orderIdIdx: index("idx_invoices_order_id").on(table.orderId),
    statusIdx: index("idx_invoices_status").on(table.status),
    expiresAtIdx: index("idx_invoices_expires_at").on(table.expiresAt),
  }),
);
...
export const invoiceItems = pgTable(
  "invoice_items",
  {
    ...
    // Customer answers for negotiated/custom products, snapshotted at
    // invoice conversion so later product-form edits cannot change history.
    customerFields: jsonb("customer_fields").notNull().default({}),
    createdAt: timestamp("created_at").notNull().defaultNow(),
  },
  (table) => ({
...
export const invoiceInventoryHolds = pgTable(
  "invoice_inventory_holds",
  {
    id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
    invoiceId: text("invoice_id").notNull().references(() => invoices.id),
    productId: text("product_id").notNull().references(() => products.id),
  ...
  (table) => ({
    invoiceIdIdx: index("idx_invoice_inventory_holds_invoice_id").on(table.invoiceId),
    activeIdx: index("idx_invoice_inventory_holds_active").on(table.releasedAt),
    baseLineUnique: uniqueIndex("uq_invoice_inventory_holds_base")
      .on(table.invoiceId, table.productId, table.branchId)
      .where(isNull(table.variantId)),
    variantLineUnique: uniqueIndex("uq_invoice_inventory_holds_variant")
      .on(table.invoiceId, table.productId, table.variantId, table.branchId)
...
export const invoicePayments = pgTable(
  ...
  (table) => ({
    ...
    statusIdx: index("idx_invoice_payments_status").on(table.status),
    pendingInvoiceUnique: uniqueIndex("uq_invoice_payments_pending_invoice")
      .on(table.invoiceId)
      .where(sql`${table.status} = 'pending'`),
  }),
);

// Per-user preferences keep the product form uncluttered without changing
// the product data or forcing every staff member into the owner's layout.
export const vendorProductFormPreferences = pgTable(
  "vendor_product_form_preferences",
  {
    id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
    storeId: text("store_id").notNull().references(() => stores.id),
    // User ids can belong to users (owners) or staff; both share the
    // vendor form, so this is intentionally a plain reference rather than
    // a users-only FK.
    userId: text("user_id").notNull(),
    visibleFields: jsonb("visible_fields").notNull().default([]),
    visibleFieldsVersion: integer("visible_fields_version").notNull().default(1),
    sectionOrder: jsonb("section_order").notNull().default([]),
    collapsedSections: jsonb("collapsed_sections").notNull().default([]),
    updatedAt: timestamp("updated_at").notNull().defaultNow(),
  },
  (table) => ({
    ownerUnique: uniqueIndex("uq_vendor_product_form_preferences_owner").on(table.storeId, table.userId),
  }),
);
...
export const orderItems = pgTable(
  "order_items",
  {
    ...
    // when the charged price differs, either from an owner override or a
    // cached offline price (priceOverridden then true). lineDiscount is a
    // per-line markdown in kobo, already reflected in lineTotal.
    ...
    // The buyer's answers to the product's vendor-defined customer fields,
    // snapshotted at order creation so a later product-form edit can't
    // rewrite history - same as invoiceItems.customerFields.
    //
    // The COLUMN has existed since INVOICE-FLOW-MIGRATION.sql, and
    // finalizePaidCheckoutAttempt has always passed `customerFields` when
    // inserting order items - but it was never declared here, so drizzle
    // silently dropped the key and every online order stored the default
    // '{}'. The vendor's and the customer's order-detail views both render
    // these (see components/ui/OrderItemModal.js), so the answers a shopper
    // typed simply never appeared. They survive on checkout_attempts.items
    // for orders placed before this was declared, if any need recovering.
    customerFields: jsonb("customer_fields").notNull().default({}),
  },
  (table) => ({
