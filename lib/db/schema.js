...
// How a tender on an order was paid. "cash" is the only one that touches
// the physical drawer (and the only one change_given is ever > 0 for).
...
export const products = pgTable(
  "products",
  {
    ...
    // Quantity price breaks (wholesale): [{ minQty, unitPrice }], sorted
    // ascending, minQty >= 2. A line's unit price is the highest tier
    // whose minQty the quantity reaches, else the discount-adjusted base
    // price - see tieredUnitPrice in lib/pricing.js. Customer-facing
...
// changeGiven is only ever > 0 for cash (customer handed over more than
// the balance). The invariant, enforced in lib/pos.js at settle:
// SUM(amount - changeGiven) === order.totalAmount (in kobo).
