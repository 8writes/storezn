...
import { failCheckoutAttemptAndReleaseStock, finalizePaidCheckoutAttempt } from "./checkoutAttempts.js";
import { sendOnlineOrderNotifications } from "./orderNotifications.js";
import { logAppError } from "./appErrorLog.js";

const STALE_AFTER_MS = 60 * 60_000;
const CLEANUP_BATCH_SIZE = 500;
const PAYMENT_RECONCILE_BATCH_SIZE = 100;
const PAYMENT_RECONCILE_MIN_AGE_MS = 30_000;
const RELEASED_PAYMENT_RECOVERY_WINDOW_MS = 24 * 60 * 60_000;

// Webhooks are the normal payment path, but Paystack can deliver them late
// or retry them after a transient failure. Verify a bounded batch of pending
// checkout attempts as a recovery path so a real payment is not left out of
// orders, analytics, payouts, or stock finalisation until the stale sweep.
export async function reconcilePendingCheckoutPayments() {
  const cutoff = new Date(Date.now() - PAYMENT_RECONCILE_MIN_AGE_MS);
  const attempts = await db
    .select()
    .from(checkoutAttempts)
    .where(and(eq(checkoutAttempts.paymentStatus, "pending"), lt(checkoutAttempts.createdAt, cutoff)))
    .orderBy(checkoutAttempts.createdAt)
  ...
  for (const attempt of attempts) {
    try {
      const transaction = await verifyTransaction(attempt.paymentReference);
      if (transaction.paymentStatus === "PAID") {
        if (transaction.amountPaid < attempt.totalAmount - 0.5) {
          await logAppError(new Error("Paystack amount mismatch during pending-payment reconciliation"), {
            source: "paystack.reconcile_amount_mismatch",
            level: "warn",
            storeId: attempt.storeId,
            metadata: { paymentReference: attempt.paymentReference, amountPaid: transaction.amountPaid, expectedAmount: attempt.totalAmount, checkoutAttemptId: attempt.id },
          });
          if (await failCheckoutAttemptAndReleaseStock(attempt)) failed += 1;
          continue;
        }

        const finalized = await finalizePaidCheckoutAttempt({ attempt, paymentReference: attempt.paymentReference, paidAt: new Date() });
        if (finalized?.created && finalized.order) {
          recovered += 1;
      ...
      // A Paystack/network failure should leave the attempt pending for the
      // next run; the stale sweep remains the final abandoned-checkout path.
      await logAppError(err, {
        source: "paystack.reconcile_pending",
        level: "warn",
        storeId: attempt.storeId,
        metadata: { paymentReference: attempt.paymentReference, checkoutAttemptId: attempt.id },
      });
    }
  }

  return { recovered, failed, checked: attempts.length };
}

// Safety net for a rare but expensive edge case: Storezn may release a
// checkout reservation after abandonment/stale cleanup while the Paystack
// authorization URL is still capable of succeeding. Webhooks and the
// customer's return page both recover that now, but this sweep covers a
// missed webhook and a buyer who closes the browser after paying.
export async function reconcileReleasedCheckoutPayments() {
  const since = new Date(Date.now() - RELEASED_PAYMENT_RECOVERY_WINDOW_MS);
  const attempts = await db
    .select()
    .from(checkoutAttempts)
    .where(and(eq(checkoutAttempts.paymentStatus, "failed"), isNull(checkoutAttempts.orderId), gt(checkoutAttempts.createdAt, since)))
    .orderBy(checkoutAttempts.createdAt)
    .limit(PAYMENT_RECONCILE_BATCH_SIZE);
  ...
  for (const attempt of attempts) {
    try {
      ...
      if (transaction.amountPaid < attempt.totalAmount - 0.5) {
        await logAppError(new Error("Paystack amount mismatch during released-payment recovery"), {
          source: "paystack.recover_released_amount_mismatch",
          level: "warn",
          storeId: attempt.storeId,
          metadata: { paymentReference: attempt.paymentReference, amountPaid: transaction.amountPaid, expectedAmount: attempt.totalAmount, checkoutAttemptId: attempt.id },
        });
      ...
      if (finalized?.created && finalized.order) {
        ...
        if (finalized.oversold) {
          oversold += 1;
          await logAppError(new Error("Released paid checkout recovered without enough stock"), {
            source: "paystack.recover_released_oversold",
            level: "warn",
            storeId: attempt.storeId,
            metadata: { paymentReference: attempt.paymentReference, checkoutAttemptId: attempt.id, orderId: finalized.order.id },
          });
        }
        await sendOnlineOrderNotifications(finalized.order, finalized.items);
      }
      ...
      await logAppError(err, {
        source: "paystack.recover_released",
        level: "warn",
        storeId: attempt.storeId,
        metadata: { paymentReference: attempt.paymentReference, checkoutAttemptId: attempt.id },
      });
  ...
  return { recovered, oversold, checked, candidates: attempts.length };
}
