...
export async function failStaleTransactions() {
  ...
  await db.transaction(async (tx) => {
    ...
    // behind a dead order for no reason once it's already been swept. The
    // order's own status moves to "abandoned" too, so it reads as a
    // never-completed checkout rather than sitting forever on "pending".
    await tx.update(orders).set({ paymentStatus: "failed", status: "abandoned" }).where(inArray(orders.id, staleIds));
  });
