...
export function buildSessionSummary({ session, orders, tenders, movements }) {
  ...
  // Net taken per method / per POS terminal (amount handed over minus any
  // change given back). Change on a non-cash tender is cash out of the
  // drawer - track it so the drawer figure isn't a mystery.
  const byMethod = {};
  const byCardProvider = {};
  let nonCashChangeOut = 0;
  for (const t of tenders) {
    const net = t.amount - Number(t.changeGiven || 0);
    byMethod[t.method] = (byMethod[t.method] || 0) + net;
    if (t.method === "card") {
      const key = (t.provider || "").trim() || "POS (unspecified)";
      byCardProvider[key] = (byCardProvider[key] || 0) + net;
    }
    if (t.method !== "cash") nonCashChangeOut += Number(t.changeGiven || 0);
  }
