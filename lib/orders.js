...
export function computeOrderTotals({ subtotal, shippingFee, commissionRatePercent, flatFee = 0, feeChargedToCustomer = false, maxCommissionAmount }) {
  ...
  if (maxCommissionAmount != null) commissionAmount = Math.min(commissionAmount, Math.max(0, maxCommissionAmount - flatFee));
  const vendorAmount = subtotal + shippingFee;
  let totalFee = commissionAmount + flatFee;
  ...
  const flatFeeAmount = Math.min(flatFee, totalFee);
  commissionAmount = Math.round((totalFee - flatFeeAmount) * 100) / 100;
  return { totalAmount, commissionAmount, flatFeeAmount, vendorPayoutAmount };
}
