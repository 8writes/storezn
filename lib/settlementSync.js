...
import { invoicePayments, orders, stores } from "./db/schema.js";
...
export async function syncStoreSettlements(store) {
  ...
  for (const settlement of successful) {
    ...
    const invoiceResult = await db
      ...
      .set({ settledAt: new Date(settlement.settlement_date || settlement.updatedAt || Date.now()) })
      .where(and(inArray(invoicePayments.paymentReference, references), eq(invoicePayments.status, "paid"), isNull(invoicePayments.settledAt)))
      .returning({ id: invoicePayments.id });
    updated += invoiceResult.length;
  }
  await db.execute(sql`
    update ${orders} o
    set settled_at = (
      select max(ip.settled_at) from ${invoicePayments} ip where ip.invoice_id = o.invoice_id and ip.status = 'paid'
    )
    where o.store_id = ${store.id}
      ...
      and not exists (
        select 1 from ${invoicePayments} ip
        where ip.invoice_id = o.invoice_id and ip.status = 'paid' and ip.settled_at is null
      )
  `);
  return updated;
}
