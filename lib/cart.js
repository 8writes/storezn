...
import { tieredUnitPrice, stripInternalProductFields } from "./pricing.js";
...
export async function getCartWithItems(cartId) {
  ...
  return rows.map((r) => {
    // Wholesale tiers apply to the base product only (a variant carries
    // its own price), same as discountPercent. costPrice is stripped -
    // this goes to the storefront client.
    const price = r.variant?.price ?? tieredUnitPrice(r.product, r.quantity);
    return {
      id: r.itemId,
      quantity: r.quantity,
