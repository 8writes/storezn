...
export async function uploadPublicFileWithMetadata(buffer, key, contentType) {
  ...
  if (publicBase) return { url: `${publicBase}/${key}` };
...
export function isOwnedUploadUrl(url, userId) {
  const key = keyFromPublicUrl(url);
  // Shared with the Cloudinary provider (see keyBelongsToUser) - this used
  // to accept only `profile/` and `review-image/` keys, so every
  // product-image and store-logo cleanup was rejected as not-owned.
