...
// must use the same one the file was uploaded under. Anything that isn't
// an image (assignment submissions, documents) goes in as "raw".
...
export async function uploadPublicFile(buffer, key, contentType) {
  ...
  return resourceType === "image"
    ? `https://res.cloudinary.com/${cloudName}/image/upload/${TRANSFORM}/${data.public_id}.${data.format}`
    : `https://res.cloudinary.com/${cloudName}/raw/upload/${data.public_id}.${data.format}`;
}
...
function infoFromUrl(url) {
  ...
  for (const resourceType of ["image", "raw"]) {
    const marker = resourceType === "image" ? `/image/upload/${TRANSFORM}/` : "/raw/upload/";
...
export function isOwnedUploadUrl(url, userId) {
  ...
  // Not anchored to the start: generateObjectKey() prefixes every key
  // with the shared storage folder (see lib/storage/index.js), so this
  // just needs "<userId>/" to appear somewhere in the path.
  return !!publicId && publicId.includes(`/${userId}/`);
}
