...
export async function uploadPublicFile(buffer, key, contentType) {
  ...
  if (resourceType === "image") return `https://res.cloudinary.com/${cloudName}/image/upload/${TRANSFORM}/${data.public_id}.${data.format}`;
  if (resourceType === "video") return `https://res.cloudinary.com/${cloudName}/video/upload/${VIDEO_TRANSFORM}/${data.public_id}.${data.format}`;
  return `https://res.cloudinary.com/${cloudName}/raw/upload/${data.public_id}.${data.format}`;
}
