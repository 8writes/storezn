...
// Walks every page, not just the first. This is what makes
// ensureSubAccount below idempotent, and a single un-paginated page meant
// that once the platform passed one page of vendors, an existing
// sub-account stopped being found - so re-linking the same bank account
// silently created a SECOND Paystack sub-account and repointed the store
// at it, splitting that vendor's settlement history in two.
const SUBACCOUNT_PAGE_SIZE = 100;
const SUBACCOUNT_MAX_PAGES = 100;

async function findSubAccount({ bankCode, accountNumber }) {
  for (let page = 1; page <= SUBACCOUNT_MAX_PAGES; page += 1) {
    const res = await fetch(`${BASE_URL}/subaccount?perPage=${SUBACCOUNT_PAGE_SIZE}&page=${page}`, { headers: headers() });
    const data = await res.json();
    if (!data.status) {
      throw new Error(data.message || "Failed to list sub-accounts");
    }
    const rows = Array.isArray(data.data) ? data.data : [];
    const match = rows.find((s) => s.settlement_bank === bankCode && s.account_number === accountNumber);
    if (match) return match;
    // A short page is the last page. Paystack's `meta.pageCount` is also
    // available, but a row count below the page size is true regardless
    // of which meta shape the endpoint returns.
    if (rows.length < SUBACCOUNT_PAGE_SIZE) return undefined;
  }
  // Ran out of pages to search rather than confirming absence - creating a
  // duplicate sub-account for a live bank account is worse than failing
  // the link and asking someone to look.
  throw new Error("Could not search Paystack sub-accounts completely; not creating a possible duplicate");
}
...
// string.
export async function listSettlements({ subaccount, from, page = 1, perPage = 100 } = {}) {
...
export async function getSettlementTransactions(settlementId, { page = 1, perPage = 100 } = {}) {
  const params = new URLSearchParams({ perPage: String(perPage), page: String(page) });
  const res = await fetch(`${BASE_URL}/settlement/${settlementId}/transactions?${params}`, { headers: headers() });
  const data = await res.json();
  if (!data.status) throw new Error(data.message || "Failed to fetch settlement transactions");
  return { items: Array.isArray(data.data) ? data.data : [], meta: data.meta || {} };
}

// Paystack can return an empty list from a sub-account settlement's
// `/transactions` endpoint even when the settlement itself succeeded. The
// account-scoped transaction list remains authoritative and includes the
// original references needed to reconcile Storezn orders.
export async function listTransactions({ subaccount, from, to, page = 1, perPage = 100 } = {}) {
  const params = new URLSearchParams({ perPage: String(perPage), page: String(page) });
  if (subaccount) params.set("subaccount", subaccount);
  if (from) params.set("from", from);
  if (to) params.set("to", to);
  const res = await fetch(`${BASE_URL}/transaction?${params}`, { headers: headers() });
  const data = await res.json();
  if (!data.status) throw new Error(data.message || "Failed to list transactions");
  return { items: Array.isArray(data.data) ? data.data : [], meta: data.meta || {} };
}
