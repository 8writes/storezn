...
// offline selling, recorded past/phone sales, cash-drawer movements,
...
export function getPlusMonthlyPrice(store, settings) {
  ...
  if (store?.subscriptionDiscountPercent != null) {
    return Math.round(basePrice * (1 - Number(store.subscriptionDiscountPercent) / 100) * 100) / 100;
  }
