"use client";
import { barcodeCandidates, barcodeMatches, normalizeBarcode } from "./barcode.js";
...
export async function catalogMeta(storeId, branchId = null) {
  try {
    const db = await openDb();
    const meta = await wrap(db.transaction(META, "readonly").objectStore(META).get(catalogScope(storeId, branchId)));
    db.close();
    return meta || null;
  } catch {
    return null;
...
export async function searchCatalogPage(storeId, q, { page = 1, pageSize = 20, branchId = null } = {}) {
  const db = await openDb();
  const scope = catalogScope(storeId, branchId);
  let all = await wrap(db.transaction(CATALOG, "readonly").objectStore(CATALOG).index("scope").getAll(scope));
  const meta = await wrap(db.transaction(META, "readonly").objectStore(META).get(scope));
  // Older clients could save an unscoped snapshot before branch-aware
  // catalogues were introduced. It is still safer to search that snapshot
  // than to show an empty product picker while offline; the current sync
  // path will replace it with the branch-scoped copy when connected.
  if (branchId && (all.length === 0 || !meta?.complete || Number(meta.count) !== all.length)) {
    all = await wrap(db.transaction(CATALOG, "readonly").objectStore(CATALOG).index("scope").getAll(catalogScope(storeId, null)));
  }
  db.close();
...
export async function findBySku(storeId, sku, branchId = null) {
  const db = await openDb();
  const scope = catalogScope(storeId, branchId);
  const candidates = barcodeCandidates(sku);
  let hit = null;
  for (const candidate of candidates) {
    hit = await wrap(db.transaction(CATALOG, "readonly").objectStore(CATALOG).index("scopeSku").get([scope, candidate]));
    if (hit) break;
  }
  if (!hit) {
    const all = await wrap(db.transaction(CATALOG, "readonly").objectStore(CATALOG).index("scope").getAll(scope));
...
export async function getCatalogProduct(storeId, productId, branchId = null) {
  try {
    const db = await openDb();
    const product = await wrap(db.transaction(CATALOG, "readonly").objectStore(CATALOG).get(`${catalogScope(storeId, branchId)}:${productId}`));
    db.close();
    return product || null;
  } catch {
    return null;
