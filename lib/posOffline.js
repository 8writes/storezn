...
export async function saveCatalog(storeId, products, branchId = null) {
  ...
  tx.objectStore(META).put({ storeId: scope, actualStoreId: storeId, branchId, count: products.length, savedAt: new Date().toISOString() });
  await txDone(tx);
  db.close();
}
...
export async function searchCatalog(storeId, q, limit = 12, branchId = null) {
  const db = await openDb();
  const all = await wrap(db.transaction(CATALOG, "readonly").objectStore(CATALOG).index("scope").getAll(catalogScope(storeId, branchId)));
  db.close();
