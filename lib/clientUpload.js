...
// Client-side helper for the authenticated vendor upload endpoint
// (POST /api/v1/uploads/file): product photos and videos, and the store
// logo/favicon. Takes a `purpose` - which is what the server uses to pick
// the size cap and the object-key prefix - and returns just a URL, leaving
// the caller to attach it to whatever record it belongs to.
