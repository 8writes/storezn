...
export async function getMarketplaceProducts({ page = 1, pageSize = 20, q, categoryName, minPrice, maxPrice, sort = "newest" } = {}) {
  ...
  const [rows, [{ total }]] = await Promise.all([
    db
      .select({
        product: products,
        store: { id: stores.id, name: stores.name, slug: stores.slug, customDomain: stores.customDomain, domainStatus: stores.domainStatus, state: stores.state },
      })
      .from(products)
  ...
  return { list: rows.map((r) => ({ ...stripInternalProductFields(r.product), store: r.store })), total };
}
