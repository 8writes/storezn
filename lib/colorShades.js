...
// Storefront header/footer render white text over this color once it's
// set (see app/storefront/[host]/layout.js's `themed`, Footer.js) - a
// white or near-white pick would make that text unreadable, so both the
// vendor settings form and the PATCH route that saves it (see
// updateVendorStoreSchema's storefrontAccentColor) reject it here before
// that can happen. Relative luminance (standard sRGB coefficients,
// un-gamma-corrected - close enough for a UI guard, not display
// calibration) above ~0.82 covers white, near-white greys, and pale/
// pastel colors bright enough to wash out white text the same way.
