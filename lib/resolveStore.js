...
export async function resolveStoreByHost(host) {
  ...
  const [row] = await db
    ...
    .limit(1);
  if (!row) {
    storeCache.set(hostname, { store: null, expiresAt: Date.now() + STORE_CACHE_TTL_MS });
    return null;
  }
  ...
  if (!slug && !isPlusStore(row.store)) {
    storeCache.set(hostname, { store: null, expiresAt: Date.now() + STORE_CACHE_TTL_MS });
    return null;
  }

  const store = { ...row.store, ownerApprovalStatus: row.ownerApprovalStatus };
  storeCache.set(hostname, { store, expiresAt: Date.now() + STORE_CACHE_TTL_MS });
  while (storeCache.size > STORE_CACHE_MAX) storeCache.delete(storeCache.keys().next().value);
  return store;
}

// A customer account belongs to exactly one store (see customers.storeId
// and its per-store unique email in lib/db/schema.js), but getUser()
// resolves a JWT without knowing which host it arrived on - so a customer
// of store A presenting their token on store B's host authenticates fine.
// Every storefront route that accepts a customer token has to reject that,
// or store A's shoppers can build carts and place orders on store B (whose
// vendor then can't see them in their own customer list, since that reads
// customers scoped to store B) and can spend store B's upload quota.
//
// Guests (no user) and vendor/staff/super_admin tokens are not customer
// identities and are left to each route's own rules.
export function isForeignCustomer(user, store) {
  return user?.role === "customer" && user.storeId !== store?.id;
}
