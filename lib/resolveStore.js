...
// A store is only visible/sellable once it's both in good standing
// (isActive, independent super_admin suspension) and its owner's identity
// is verified (ownerApprovalStatus, see resolveStoreByHost above).
export function isStoreLive(store) {
  return !!store?.isActive && store?.ownerApprovalStatus === "approved";
}
