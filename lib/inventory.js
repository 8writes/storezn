...
export async function resolveFulfillingBranch(tx, storeId, items, { preferredBranchId } = {}) {
  ...
  if (preferredBranchId) {
    const preferredIndex = storeBranches.findIndex((branch) => branch.id === preferredBranchId);
    if (preferredIndex >= 0) storeBranches.unshift(storeBranches.splice(preferredIndex, 1)[0]);
  }
