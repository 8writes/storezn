...
export async function reserveStock(tx, items) {
  for (const item of items) {
    const [updated] = await tx
      ...
      .where(and(branchStockCondition(item.productId, item.variantId, item.branchId), isNotNull(productBranchStock.stock), sql`${productBranchStock.stock} >= ${item.quantity}`))
    ...
    if (!updated) {
      ...
      // this branch" - nothing to reserve, not an error.
...
export async function seedBranchStockForNewItem(tx, { storeId, productId, variantId, initialBranchId, initialStock }) {
...
// branches, new product).
export async function seedNewBranchStock(tx, storeId, branchId) {
  const storeProducts = await tx.select({ id: products.id }).from(products).where(eq(products.storeId, storeId));
  for (const product of storeProducts) {
    await tx.insert(productBranchStock).values({ productId: product.id, variantId: null, branchId, stock: 0 });
    const variants = await tx.select({ id: productVariants.id }).from(productVariants).where(eq(productVariants.productId, product.id));
    for (const variant of variants) {
      await tx.insert(productBranchStock).values({ productId: product.id, variantId: variant.id, branchId, stock: 0 });
    }
  }
}
