...
// Abbreviated count for stat tiles that would otherwise wrap / overflow
// once a number gets long: 1234 -> "1.2K", 3_400_000 -> "3.4M". Pair with
// a `title` of the full value (see StatCard) so the exact figure is one
// hover / tap away. Under 1,000 stays exact.
export function compactNumber(value) {
  const n = Number(value) || 0;
  const abs = Math.abs(n);
  if (abs < 1000) return n.toLocaleString("en-NG");
  const units = [
    ["T", 1e12],
    ["B", 1e9],
    ["M", 1e6],
...
// Same idea for money: exact up to ₦99,999.99, then "₦1.2M".
export function compactCurrency(amount) {
  const n = Number(amount) || 0;
  if (Math.abs(n) < 100000) return formatCurrency(n);
...
// time-of-day together. Always a 12-hour clock with AM/PM (hour12 forced
// so it doesn't fall to 24h on environments whose en-NG ICU data does).
...
// Just the time-of-day from a Date/ISO timestamp, 12-hour with AM/PM.
