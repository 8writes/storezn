...
// How many reverse proxies sit in front of the app. The deployment is a
// single nginx/Caddy in front of pm2 (see scripts/deploy.sh), so 1 - raise
// it only if another proxy (a CDN, a load balancer) is added in front.
function trustedHopCount() {
...
// "unknown" means every anonymous caller shares ONE bucket, which turns
// each limiter below from "throttle this client" into "let any client
// exhaust the limit for everybody" - a 10/min checkout cap becomes a
// platform-wide 10/min. That is a worse failure than trusting a header, so
// say so loudly in production rather than failing silently.
function warnMissingProxyTrust() {
  if (warnedAboutProxyTrust || process.env.NODE_ENV !== "production") return;
  warnedAboutProxyTrust = true;
  console.error(
    "rateLimit: TRUST_PROXY_HEADERS is not 'true', so every anonymous request shares one rate-limit bucket. " +
      "Set TRUST_PROXY_HEADERS=true (the reverse proxy sets X-Forwarded-For) to restore per-client limits.",
  );
}
...
function getClientIp(req) {
  ...
  if (process.env.TRUST_PROXY_HEADERS === "true") {
    ...
    if (forwarded) {
      // Read from the RIGHT, not the left. A client can send its own
      // X-Forwarded-For and our proxy appends to it, so the leftmost entry
      // is attacker-controlled (send "X-Forwarded-For: 1.2.3.4" and get a
      // fresh bucket per request). The entry our own trusted proxy
      // appended is the real peer, and it sits `trustedHopCount()` from
      // the end.
      const hops = forwarded.split(",").map((part) => part.trim()).filter(Boolean);
      const peer = hops[hops.length - trustedHopCount()];
      if (peer) return peer.slice(0, 128);
    }
    const realIp = req.headers.get("x-real-ip");
    if (realIp) return realIp.trim().slice(0, 128);
  }
  warnMissingProxyTrust();
  return "unknown";
}
