...
export default function VendorAnalyticsPage() {
  ...
  // when a fetch settles; the filter handlers below switch it back on, so
  // it is never set synchronously inside this effect. `alive` drops a reply
  // whose date range or branch has since changed.
  useEffect(() => {
    if (!token || !storeId || !from || !to) return undefined;
    let alive = true;
    ...
    apiFetch(`/api/v1/vendor/stores/${storeId}/analytics?${params}`)
      ...
      .catch((err) => {
        if (alive) toast.error(err.message || "Failed to load analytics");
      })
      .finally(() => {
  ...
  }, [apiFetch, token, storeId, from, to, branchId, channel]);

  // Every filter change refetches, so each one turns the spinner back on -
  // the fetch effect above only ever clears it. Keeping that in the
  // handlers is what lets the effect stay free of a synchronous setState.
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-surface border border-slate-200 rounded-sm p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 items-end gap-3 w-full">
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            ...
            <StatCard
              ...
              sub={
                <ChangeBadge percent={data.summary.orderCountChangePercent} />
              }
            />
            <StatCard
              icon={Receipt}
              label="Avg. order value"
              value={compactCurrency(data.summary.averageOrderValue)}
              title={formatCurrency(data.summary.averageOrderValue)}
            />
            <StatCard
              icon={Package}
              label="Units sold"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-x-5 gap-y-4 text-sm">
              <div>
                ...
                <p className="text-lg font-bold text-slate-900">
                  {data.invoices?.requests || 0}
                </p>
                <p className="text-xs text-slate-600">
                  {data.invoices?.offlineRequests || 0} offline
                </p>
              </div>
              <div>
                <p className="text-slate-600">Invoices sent</p>
                <p className="text-lg font-bold text-slate-900">
                  {data.invoices?.total || 0}
                </p>
                <p className="text-xs text-slate-600">
                  {data.invoices?.sent || 0} awaiting payment
                </p>
              </div>
              <div>
                <p className="text-slate-600">Partially paid</p>
                <p className="text-lg font-bold text-slate-900">
                  {data.invoices?.partiallyPaid || 0}
                </p>
              </div>
              <div>
                <p className="text-slate-600">Paid in full</p>
                <p className="text-lg font-bold text-slate-900">
                  {data.invoices?.paid || 0}
                </p>
              </div>
              <div>
                <p className="text-slate-600">Invoice payments</p>
                <p
                  className="text-lg font-bold text-green-700"
                  title={formatCurrency(data.invoices?.received || 0)}
                >
                  {compactCurrency(data.invoices?.received || 0)}
                </p>
              </div>
              <div>
                ...
                <p
                  ...
                  title={formatCurrency(data.invoices?.outstanding || 0)}
                >
                  {compactCurrency(data.invoices?.outstanding || 0)}
                </p>
              </div>
            </div>
          </section>
              ...
              <Chart
                ...
                data={{
                  ...
                  datasets: [
                    {
                      label: "Orders",
                      data: data.daily.map((d) => d.orderCount),
                ...
                <ul className="space-y-2.5">
                  {data.statusBreakdown
                    ...
                    .map((s) => (
                      ...
                      >
                        <Badge color={STATUS_COLOR[s.status] || "slate"}>
                          {s.status.replace("_", " ")}
                        </Badge>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-surface border border-slate-200 rounded-sm overflow-hidden">
              <p className="text-sm font-semibold text-slate-700 px-5 py-4 border-b border-slate-100">
                Top products
              </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-surface border border-slate-200 rounded-sm overflow-hidden">
              ...
              {data.topCustomers.length === 0 ? (
                <p className="text-sm text-slate-400 py-10 text-center">
                  No registered-customer sales in this period.
                </p>
              ) : (
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {data.topCustomers.map((c) => (
                      <tr key={c.customerId}>
                        ...
                        <td className="px-5 py-3 text-slate-800 text-right whitespace-nowrap">
                          {c.orderCount} orders
                        </td>
                        <td className="px-5 py-3 font-semibold text-slate-900 text-right whitespace-nowrap">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-surface border border-slate-200 rounded-sm p-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {["online", "offline"].map((ch) => {
                    const row = data.channelBreakdown.find(
                      (c) => c.channel === ch,
                    );
                    return (
              ...
              <div className="bg-surface border border-slate-200 rounded-sm p-5 space-y-3">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="flex items-center gap-1.5 text-slate-700">
                    <Package size={14} /> Live products
                  </span>
                  <span className="font-medium text-slate-900">
                    {data.products.live} / {data.products.total}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="flex items-center gap-1.5 text-slate-700">
                    <AlertTriangle
                      size={14}
                      className={
                        data.products.lowStock > 0 ? "text-amber-500" : ""
                      }
                    />{" "}
                    Low stock
                  </span>
                  <span
                    className={`font-medium ${data.products.lowStock > 0 ? "text-amber-600" : "text-slate-900"}`}
                  >
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="flex items-center gap-1.5 text-slate-700">
                    <AlertTriangle
                      size={14}
                      className={
                        data.products.outOfStock > 0 ? "text-red-500" : ""
                      }
                    />{" "}
                    Out of stock
                  </span>
                  <span
                    className={`font-medium ${data.products.outOfStock > 0 ? "text-red-600" : "text-slate-900"}`}
                  >
                    {data.products.outOfStock}
                  </span>
                </div>
                {(data.products.expiringSoon > 0 ||
                  data.products.expired > 0) && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="flex items-center gap-1.5 text-slate-700">
                      <CalendarClock
                        size={14}
                    ...
                    <span
                      className={`font-medium ${data.products.expired > 0 ? "text-red-600" : "text-amber-600"}`}
                    >
                      {data.products.expiringSoon}
                      {data.products.expired > 0 && (
                        <span className="text-xs font-normal text-slate-800">
                          {" "}
                          ({data.products.expired} expired)
                        </span>
                      )}
                    </span>
                  </div>
                ...
                <div className="flex items-center justify-between text-sm pt-2 border-t border-slate-100">
                  <span className="flex items-center gap-1.5 text-slate-700">
                    <RotateCcw size={14} /> Refund requests (period)
                  </span>
                  <span className="font-medium text-slate-900">
                    {data.refunds.pending} pending, {data.refunds.approved}{" "}
                    approved
                  </span>
                </div>
              </div>
