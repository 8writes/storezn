...
const invoiceNumber = () =>
  `INV-${Date.now().toString(36).toUpperCase()}-${crypto.randomUUID().slice(0, 6).toUpperCase()}`;
...
export async function GET(req, { params }) {
  ...
  if (!user || !store || !canManageStore(user, store))
  ...
  const itemRows = invoiceIds.length
    ? await db
        .select({
          ...
          variantLabel: invoiceItems.variantLabel,
        })
        .from(invoiceItems)
        .where(inArray(invoiceItems.invoiceId, invoiceIds))
    : [];
  const itemSearchByInvoice = new Map();
  for (const item of itemRows) {
    const label = [item.productName, item.variantLabel]
      .filter(Boolean)
      .join(" ");
    itemSearchByInvoice.set(item.invoiceId, [
      ...(itemSearchByInvoice.get(item.invoiceId) || []),
      label,
    ]);
  }
  ...
  return NextResponse.json({
    invoices: rows.map(({ invoice, order }) => ({
      ...invoice,
      productNames: itemSearchByInvoice.get(invoice.id) || [],
    ...
    feePolicy: {
      commissionRatePercent:
        store.commissionRatePercent ??
        settings?.defaultCommissionRatePercent ??
        5,
...
async function handlePost(req, { params }) {
  ...
  const [request] = await db
    ...
    .where(
      and(
        eq(invoiceRequests.id, result.data.requestId),
        eq(invoiceRequests.storeId, storeId),
      ),
    )
    .limit(1);
  if (!request)
  ...
  if (["cancelled", "expired", "converted"].includes(request.status))
    return NextResponse.json(
  ...
  const requestedItems = await db
    ...
    .where(eq(invoiceRequestItems.requestId, request.id));
  ...
  const productIds = [
    ...new Set(result.data.items.map((item) => item.productId)),
  ];
  const variantIds = [
    ...new Set(result.data.items.map((item) => item.variantId).filter(Boolean)),
  ];
  ...
  for (const item of result.data.items) {
    const requestItem = requestItemByKey.get(
      `${item.productId}:${item.variantId || ""}`,
    );
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Invoice items must come from the selected request" },
        { status: 400 },
      );
    }
    if (item.quantity > requestItem.quantity)
      return NextResponse.json(
        { error: `Quantity for ${product.name} exceeds the request` },
        { status: 400 },
      );
    lines.push({
      product,
      ...
      customerFields: requestItem.customerFields || {},
    });
  }
  ...
  const commissionRatePercent =
    store.commissionRatePercent ?? settings?.defaultCommissionRatePercent ?? 5;
  ...
  if (
    !feeChargedToCustomer &&
    configuredFlatFee > totals.flatFeeAmount + 0.001
  ) {
    return NextResponse.json(
      {
        error:
          "This invoice total is too low for the store to absorb the full platform fee. Increase the quoted price before creating the invoice.",
      },
      { status: 422 },
    );
  }
  const depositAmount =
    result.data.plan === "deposit"
      ? Math.round(totals.totalAmount * 50) / 100
      : totals.totalAmount;
  const invoiceId = crypto.randomUUID();
  const orderId = crypto.randomUUID();
  const now = new Date();
  const expiresAt =
    result.data.expiresAt || new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);
  if (expiresAt.getTime() <= now.getTime())
    return NextResponse.json(
      { error: "Invoice expiry must be in the future" },
      { status: 400 },
    );
  if (expiresAt.getTime() > now.getTime() + 90 * 24 * 60 * 60 * 1000)
    return NextResponse.json(
      { error: "Invoice expiry cannot be more than 90 days away" },
      { status: 400 },
    );
  const physicalLines = lines.filter(
    (line) => line.product.productType === "physical",
  );
  const invoice = {
    id: invoiceId,
    storeId,
  ...
  try {
    await db.transaction(async (tx) => {
      const [lockedRequest] = await tx
        ...
        .where(
          and(
            eq(invoiceRequests.id, request.id),
            eq(invoiceRequests.storeId, storeId),
          ),
        )
        .for("update")
      ...
      if (
        !lockedRequest ||
        ["cancelled", "expired", "converted"].includes(lockedRequest.status)
      )
        throw new InvoiceConflictError("This request is no longer open");
      ...
      if (physicalLines.length > 0) {
        ...
        await tx.insert(invoiceInventoryHolds).values(
          physicalLines.map((line) => ({
            invoiceId,
            productId: line.product.id,
            variantId: line.variant?.id || null,
      ...
      await tx.insert(invoiceItems).values(
        lines.map((line) => ({
          ...
          customerFields: line.customerFields,
        })),
      );
      ...
      await tx
        ...
        .where(eq(invoiceRequests.id, request.id));
    });
  } catch (error) {
    if (
      error instanceof InvoiceConflictError ||
      (error?.code === "23505" &&
        error?.constraint === "uq_invoices_request_id")
    )
      return NextResponse.json(
