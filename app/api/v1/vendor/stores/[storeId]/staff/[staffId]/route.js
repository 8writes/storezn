...
export async function PATCH(req, { params }) {
  ...
  const staffName = `${staffRow.firstName || ""} ${staffRow.lastName || ""}`.trim() || staffRow.email;
  after(() =>
    logStoreActivity({
      storeId,
      actor: user,
      action: "staff.branch",
      summary: `Assigned ${staffName} to ${branchName}`,
      targetType: "staff",
      targetId: staffId,
      metadata: { staffName, branchId: branchId || null, branchName },
    }),
  );
...
export async function DELETE(req, { params }) {
  ...
  after(() =>
    logStoreActivity({
      ...
      actor: user,
      action: "staff.remove",
      summary: `Removed staff member ${staffName}`,
      targetType: "staff",
      targetId: staffId,
      metadata: { staffName, email: staffRow.email },
    }),
  );
