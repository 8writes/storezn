...
import { invoiceRequestItems, invoiceRequests, productVariants, products } from "../../../../../lib/db/schema.js";
...
async function handlePost(req) {
  ...
  for (const item of result.data.items) {
    ...
    items.push({ product, variant, quantity: item.quantity, customerFields: answers.data });
  }

  const customerId = user?.role === "customer" && user.storeId === store.id ? user.id : null;
  if (!customerId && !result.data.guestEmail) {
    return NextResponse.json({ error: "Email is required so the seller can send your invoice" }, { status: 400 });
  }
