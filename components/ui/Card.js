...
// `bg-surface border border-slate-200 rounded-sm p-4` - they migrate to
...
export function Card({ as: Tag = "div", pad = "md", interactive = false, className = "", children, ...props }) {
  return (
    <Tag
      className={`bg-surface border border-slate-200/80 rounded-sm shadow-xs ${pads[pad] ?? pads.md}
